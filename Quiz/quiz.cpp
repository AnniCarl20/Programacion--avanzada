...
int main (){
    ...
    do{          //Do while para que siga preguntando al usuario si quiere continuar
...
void funcionVocales(){
    ...
    int n1=5;
    int n2=6;
    puntero1 = &vocales1[0];          //Igualo punteros a direcciones de memoria
    puntero2 = &vocales2[0];

    for (int i=0; i<5; i++){
        int contador=1;
        for (int j=0; j<6; j++){
            if (*(puntero1+i) == *(puntero2+j)){     //puntero [i]
                contador++;
            }
