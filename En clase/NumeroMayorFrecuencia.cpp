...
Autor: Annika Carl
Fecha: 27/01/2023
Problema: Crear un arreglo, imprimir el número que más se repita y la frecuencia.
*********************************************************************************/
...
int main(){
	...
	funcion (arreglo, tam);         //Llamo la función
...
void funcion(int arreglo[], int tam){
	...
	for(int i=0; i<tam; i++){
		...
		aux = 0;
		for(int j=i; j<tam; j++){
			if(temporal == arreglo[j]){        //Si ese valor coincide con otro más adelante
				aux++;                         //Aumento el contador (como ambos comienzan en 0, compara la primera posicion con la primera
			}                                  //Por eso no pongo contador = 1
		}
		if(aux >= frecuencia){                 //Algoritmo de comparación de números mayores
			frecuencia = aux;
			numero = temporal;
		}
	}
